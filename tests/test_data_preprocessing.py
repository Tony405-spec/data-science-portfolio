...
class TestDataPreprocessing:
    def setup_method(self):
        ...
        self.test_df = pd.DataFrame(
            {"numeric_col": [1, 2, np.nan, 4, 5], "categorical_col": ["A", "B", None, "A", "C"], "target": [0, 1, 0, 1, 0]}
        )
    ...
    def test_clean_data_whitespace(self):
        ...
        df_with_whitespace = pd.DataFrame({"string_col": ["  hello  ", "world  ", "  test"]})
    ...
    def test_scale_features_standard(self):
        ...
        # Check that scaled data has mean approx 0 and std approx 1 (population std)
        assert abs(df_scaled["col1"].mean()) < 1e-10
        assert abs(df_scaled["col1"].std(ddof=0) - 1) < 1e-10
